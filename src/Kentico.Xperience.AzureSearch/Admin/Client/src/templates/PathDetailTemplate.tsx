...
export const PathDetailTemplate = ({
  aliasPath,
  columns,
}: PathDetailPageProps): JSX.Element => {
  ...
  const totalPages =
    tableData.totalCount % templateParameters.pageSize > 0
      ? Math.trunc(tableData.totalCount / templateParameters.pageSize) + 1
  ...
  usePageCommand<LoadDataResult, TemplateParameters>(
    ListingCommands.LoadData,
    {
      executeOnMount: true,
      after: (result) => {
        if (result !== undefined) {
